...
async function copySelectedRoles(sourceGuild, targetGuild, roleIds, requesterTag) {
...
async function runCopyWizard(interaction, config) {
  ...
  function renderConfirmStage(targetGuild) {
    const embed = new EmbedBuilder()
      ...
      .setColor(0xffa500)
      .setDescription(config.confirmText(selected.size, targetGuild));
    const row = new ActionRowBuilder().addComponents(
    ...
    try {
      if (stage === 'select') {
        ...
        if (i.customId === `${prefix}_confirm`) {
          ...
          try {
            const resultEmbed = await config.execute(targetGuild, [...selected]);
...
async function handleRoleCopy(interaction) {
  ...
  return runCopyWizard(interaction, {
    ...
    execute: async (targetGuild, selectedIds) => {
      const { created, skipped, failed } = await copySelectedRoles(interaction.guild, targetGuild, selectedIds, interaction.user.tag);
