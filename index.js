...
async function copySelectedRoles(sourceGuild, targetGuild, roleIds, requesterTag, anchorRoleId = 'NONE') {
  ...
  const created = [];
  const createdPairs = []; // { sourceRole, newRole } — used to rebuild hierarchy afterwards
  const skipped = [];
  const failed = [];
  ...
  for (const role of roles) {
    ...
    try {
      ...
      created.push(newRole);
      createdPairs.push({ sourceRole: role, newRole });
    } catch (err) {
      failed.push(`"${role.name}" — Discord error: ${err.message}`);
    }
  }

  // Reposition. IMPORTANT: Discord's bulk role-position endpoint gets unreliable
  // (roles ending up reversed, existing roles ignored) if you only send positions
  // for the NEW roles. To get correct, predictable results we instead compute the
  // COMPLETE final top-to-bottom order (every reorderable existing role + the new
  // ones spliced in) and send that whole list in a single call.
  if (created.length > 0 && targetBotMember) {
    try {
      await targetGuild.roles.fetch(); // refresh so the cache includes the roles we just created
      const botHighestPos = targetBotMember.roles.highest.position;
      const botCeiling = botHighestPos - 1; // highest position we're allowed to hand out

      const newRoleIds = new Set(createdPairs.map(p => p.newRole.id));

      // Baseline: every role we're actually allowed to reorder, current order, top -> bottom.
      const existingList = [...targetGuild.roles.cache.values()]
        .filter(r => r.id !== targetGuild.id && !r.managed && r.position < botHighestPos && !newRoleIds.has(r.id))
        .sort((a, b) => b.position - a.position);

      // New roles, in the same relative order they had in the SOURCE server (top -> bottom).
      const createdTopToBottom = [...createdPairs].sort((a, b) => b.sourceRole.position - a.sourceRole.position);

      let finalOrder;

      if (anchorRoleId === 'AUTO') {
        // Walk the SOURCE guild's full role list top -> bottom (its real hierarchy,
        // not just the roles that were selected) and thread every newly created role
        // into the exact same relative slot — using any same-named role that already
        // exists in the target as a checkpoint to re-anchor against as we go.
        const allSourceRolesDesc = [...sourceGuild.roles.cache.values()]
          .filter(r => r.id !== sourceGuild.id)
          .sort((a, b) => b.position - a.position);

        finalOrder = [...existingList];
        let cursorIndex = -1; // -1 = insert at the very top of the manageable range

        for (const sRole of allSourceRolesDesc) {
          const pair = createdTopToBottom.find(p => p.sourceRole.id === sRole.id);
          if (pair) {
            const insertPos = cursorIndex + 1;
            finalOrder.splice(insertPos, 0, pair.newRole);
            cursorIndex = insertPos;
            continue;
          }
          const anchor = existingTargetByName.get(sRole.name.toLowerCase());
          if (anchor) {
            const idx = finalOrder.indexOf(anchor);
            if (idx !== -1) cursorIndex = idx;
          }
        }
      } else {
        let insertAfterIndex;
        if (anchorRoleId && anchorRoleId !== 'NONE') {
          const anchorRole = existingList.find(r => r.id === anchorRoleId);
          insertAfterIndex = anchorRole ? existingList.indexOf(anchorRole) : -1;
        } else {
          insertAfterIndex = existingList.length - 1; // bottom of the manageable range
        }
        finalOrder = [...existingList];
        finalOrder.splice(insertAfterIndex + 1, 0, ...createdTopToBottom.map(p => p.newRole));
      }

      const positions = finalOrder.map((role, idx) => ({
        role: role.id,
        position: Math.max(1, botCeiling - idx)
      }));
      await targetGuild.roles.setPositions(positions);
    } catch (err) {
      failed.push(`Hierarchy ordering step failed: ${err.message} (roles were created, order may need manual fixing)`);
